...
class SMS {
    ...
    {   
        ...
        $username         = App::loadConf('app/meilian')['username'];  //用户名
        
        $password_md5     = md5(App::loadConf('app/meilian')['password_md5']);  //32位MD5密码加密，不区分大小写
        
        $apikey           = App::loadConf('app/meilian')['apikey'];  //apikey秘钥（请登录 http://m.5c.com.cn 短信平台-->账号管理-->我的信息 中复制apikey）
        
        $mobile           = $phone;  //手机号,只发一个号码：13800000001。发多个号码：13800000001,13800000002,...N 。使用半角逗号分隔。
        $rand             = mt_rand(102300, 999999);
        $content          = '您好，您的验证码是：'.$rand.App::loadConf('app/meilian')['website'];  //要发送的短信内容，特别注意：签名必须设置，网页验证码应用需要加添加【图形识别码】。
        //$content = iconv("UTF-8","GBK",$content);
        $contentUrlEncode = urlencode($content);//执行URLencode编码  ，$content = urldecode($content);解码
        
        //return $contentUrlEncode;
        ...
        //$url = "http://115.28.23.78/api/send/index.php?";
        $data=array
        (
            'username'=>$username,
