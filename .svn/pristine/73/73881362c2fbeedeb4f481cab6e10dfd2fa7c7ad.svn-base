...
(function(ROOT, struct, undefined){
     ...
     * 从translate3d中提取位置
     */
    function getTranslate3dPostion(elem, prop){
    	var transformStr = elem.style['transform'],
    		transformArr;
    	
        transformArr = transformStr.match(/\((.*)\)/);
        if(transformArr.length != 2){
        	...
        	if(transformArr.length != 3){
        		...
        		if(prop == 'left'){
            		//向左
        			return parseFloat(transformArr[0]) || 0;
            	}else{
            		//向上
            		return parseFloat(transformArr[1]) || 0;
            	}
        	}
        }
    }
    ...
    struct.prototype={
        ...
        resize:function(){
            ...
            if(this.direction == 0){
            	//向左
            	css['transform'] = 'translate3d(' + this.getPos(this.current) + 'px, 0, 0)';
            }else{
            	css['transform'] = 'translate3d(0, ' + this.getPos(this.current) + 'px, 0)';
            }
        ...
        slide:function(_index){
            var self=this,
                ...
                curPos=getTranslate3dPostion(this.container, dir?'top':'left')||0,
                type=dir?'top':'left',
                css={},tarPos;
            ...
            if(transition){
                ...
                css[transition]='transform '+duration+'ms ease';
                ...
                if(dir == 0){
                	//向左
                	css['transform'] = 'translate3d(' + tarPos + 'px, 0, 0)';
                }else{
                	css['transform'] = 'translate3d(0, ' + tarPos + 'px, 0)';
                }
                //END
            ...
            function ani(){
                ...
                if(dir == 0){
                	//向左
                	self.container.style['transform'] = 'translate3d(' + cp + 'px, 0, 0)';
                }else{
                	self.container.style['transform'] = 'translate3d(0, ' + cp + 'px, 0)';
                }
                //END
                if(offset==duration){
                    self.end();
        ...
        handleEvent:function(oldEvent){
            ...
            switch(ev.eventCode){
                case 2:
                    if(canDrag&&this.rect){
                        ...
                        if(this.drag){
                            ...
                            if(dir == 0){
                            	//向左
                            	this.container.style['transform'] = 'translate3d(' + (this.startPos+offset) + 'px, 0, 0)';
                            }else{
                            	this.container.style['transform'] = 'translate3d(0, ' + (this.startPos+offset) + 'px, 0)';
                            }
                            //END
                ...
                case 3:
                    if(canDrag){
                        ...
                        if(ev.length&&(ev.eventCode==1||this.drag)){
                            ...
                            this.startPos=getTranslate3dPostion(this.container, type);
                            ...
                            if(this.direction == 0){
                            	//向左
                            	this.container.style['transform'] = 'translate3d(' + this.startPos + 'px, 0, 0)';
                            }else{
                            	this.container.style['transform'] = 'translate3d(0, ' + this.startPos + 'px, 0)';
                            }
                ...
                case 8:
                    ...
                    */
                	if(oldEvent.propertyName=='transform'){
                        this.container.style[transition]='none';
                        this.end();
                    }
                    break;
